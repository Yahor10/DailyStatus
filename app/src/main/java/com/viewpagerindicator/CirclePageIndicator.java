...
public class CirclePageIndicator extends View implements PageIndicator {
	...
	private final Paint mPaintTextStroke = new Paint(ANTI_ALIAS_FLAG);
	...
	public CirclePageIndicator(Context context, AttributeSet attrs, int defStyle) {
		...
		mLeftPadding = res
				.getDimension(R.dimen.default_circle_text_indicator_left_padding);
		mBottomPadding = res
				.getDimension(R.dimen.default_circle_text_indicator_bottom_padding);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			...
			if (pageFillRadius != mRadius) {
				...
				canvas.drawText(Integer.toString(iLoop + 1), dX - mLeftPadding, dY
						+ mBottomPadding, mPaintTextStroke);
			}
		}
		...
		String currentPage = Integer.toString(mCurrentPage + 1);
		canvas.drawCircle(dX, dY, mRadius - 1, mPaintFill);

		canvas.drawText(currentPage, dX - mLeftPadding, dY + mBottomPadding,
				mPaintStroke);

		if (drawText) {
			canvas.drawText(currentPage, dX - mLeftPadding,
					dY + mBottomPadding, mPaintStroke);
		}
	}
